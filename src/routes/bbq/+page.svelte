...
<div class="m-4">
    <a class="border rounded px-2 py-1" href="/dog-mountain">Go Back</a>
</div>

<main class="flex flex-col space-y-4 justify-center items-center ">
    ...
    {#if dogsRemaining > 0}
            ...
            <table >
                <tbody>
                    <tr>
                        ...
                        <td class="text-left w-48">{simulationTime.toLocaleDateString()} {simulationTime.toLocaleTimeString()}</td>
                    </tr>
                    <tr>
                        <td class="pr-2 text-right">Dogs eaten</td>
                        <td class="text-left w-48">{dogFormState.state.totalDogs - dogsRemaining}</td>
                    </tr>
                    <tr>
                        <td class="pr-2 text-right">Dogs remaining</td>
                        <td class="text-left w-48">{dogsRemaining}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        >
            ...
            >
                ...
                <img src="/man.png" alt="Hungry human"
                    height={manHeight} width={manWidth}
                    class="absolute"
                    style={`
                        height: ${manHeight}px;
                        width: ${manWidth}px;
            ...
            {#each inFlightDogs.entries() as [id, dog] (id)}
                <img src="/hotdog.png" alt="Hotdog" width={dogWidth} height={dogHeight}
        ...
        {#if autodogEnabled}
            <div class="m-auto">
                <p>
                    By default, autodog runs in real time. In other words it will spawn hot dogs
                    at a rate of {dogsPerHour.toFixed(3)} per hour.
                </p>
                <label for="dog-rate" class="flex items-center space-x-2">
                    <input 
                        class="w-92"
                        type="range" name="dog-rate" id="dog-rate" 
                        min="0"
                        max="7"
                        step="1"
        ...
        {:else}
            <button class="border rounded px-8 py-6 cursor-pointer" onmousedown={spawnDog}>Another one wouldn't hurt</button>
        {/if}
